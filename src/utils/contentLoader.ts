/* Content loader — bundles raw .txt files from the repository `resources` folder using Vite's import.meta.glob.
   Exports a helper to load all game files and parse them into
...
export async function loadResources(kind: 'games' = 'games'): Promise<ParsedFile[]> {
  ...
  const modules = import.meta.glob('../../resources/games/*.txt', { as: 'raw' }) as Record<string, () => Promise<string>>;
